...
function make_plyr(x,y,z,angle)
	...
	states={
		drive=function()
			...
			return function(self)
				...
				local turn_scale=1
				-- regular drive
				if(btn(2)) acc+=0.05 turn_scale=0.7 left_v=1 right_v=1
				if(btn(3)) acc-=0.05 turn_scale=0.7 left_v=-1 right_v=-1
				
				-- rotation
				if(btn(0)) da=-0.01 left_v=-turn_scale right_v=turn_scale
				if(btn(1)) da=0.01 left_v=turn_scale right_v=-turn_scale
				...
				acc*=0.7
				left_v*=0.7
				if(abs(left_v)<0.1) left_v=0
				right_v*=0.7
				if(abs(right_v)<0.1) right_v=0
			end
		end,
		flip=function(dir)
	...
	local left_thread,right_thread=0,0
	return {
		x=x,
		y=y,
		w=0.4,
		...
		draw=function(self)
			...
			sspr(40+2*flr((left_thread%3+3)%3),32,2,7,58,111)
			sspr(40+2*flr((right_thread%3+3)%3),32,2,7,64,111,2,7,true)
			left_thread+=left_v
			right_thread+=right_v
			pal()
...
00000000000000000000000000660000000000006dd56d0000007777700000000888880000000000333333333333333300000000000000076d50000000000000
...
00006df5dfd600000015650000000000000000000000000080000088888000008000000077dddddddddd149dddddd17700000000000000000000000000000000
